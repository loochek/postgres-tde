...
// Fixture class.
class PostgresTDETest
    : public ::testing::TestWithParam<
          std::tuple<std::function<void(PostgresTDE*, Buffer::Instance&, bool)>,
                     std::function<uint32_t(const PostgresTDE*)>>> {
public:
  PostgresTDETest() {

    PostgresTDEConfig::PostgresTDEConfigOptions config_options{
        stat_prefix_, true, false,
        envoy::extensions::filters::network::postgres_proxy::v3alpha::
            PostgresProxy_SSLMode_DISABLE};

    config_ = std::make_shared<PostgresTDEConfig>(config_options, scope_);
    filter_ = std::make_unique<PostgresTDE>(config_);
  ...
  std::unique_ptr<PostgresTDE> filter_;
  PostgresTDEConfigSharedPtr config_;
...
TEST_F(PostgresTDETest, NewConnection) {
...
TEST_P(PostgresTDETest, ReadData) {
...
INSTANTIATE_TEST_SUITE_P(ProcessDataTests, PostgresTDETest,
                         ::testing::Values(std::make_tuple(&PostgresTDE::onData,
                                                           &PostgresTDE::getFrontendBufLength),
                                           std::make_tuple(&PostgresTDE::onWrite,
                                                           &PostgresTDE::getBackendBufLength)));

// Test generates various postgres payloads and feeds them into filter.
// It expects that certain statistics are updated.
TEST_F(PostgresTDETest, BackendMsgsStats) {
...
TEST_F(PostgresTDETest, ErrorMsgsStats) {
  // Pretend that startup message has been received.
  static_cast<DecoderImpl*>(filter_->getDecoder())->state(DecoderImpl::State::InSyncState);
...
TEST_F(PostgresTDETest, NoticeMsgsStats) {
  // Pretend that startup message has been received.
  static_cast<DecoderImpl*>(filter_->getDecoder())->state(DecoderImpl::State::InSyncState);
...
TEST_F(PostgresTDETest, EncryptedSessionStats) {
...
// Postgres metadata.
TEST_F(PostgresTDETest, MetadataIncorrectSQL) {
  // Pretend that startup message has been received.
  static_cast<DecoderImpl*>(filter_->getDecoder())->state(DecoderImpl::State::InSyncState);
  setMetadata();
...
TEST_F(PostgresTDETest, QueryMessageMetadata) {
  // Pretend that startup message has been received.
  static_cast<DecoderImpl*>(filter_->getDecoder())->state(DecoderImpl::State::InSyncState);
  setMetadata();
...
// Decoder::Stopped.
TEST_F(PostgresTDETest, TerminateSSL) {
...
TEST_F(PostgresTDETest, UpstreamSSL) {
  EXPECT_CALL(read_callbacks_, connection()).WillRepeatedly(ReturnRef(connection_));
...
TEST_F(PostgresTDETest, UpstreamSSLStats) {
